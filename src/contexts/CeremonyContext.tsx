...
const CEREMONIES_KEY = "awardscut.ceremonies.v1";
...
export function CeremonyProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const loadCeremonies = async () => {
      ...
      let storedCeremonies = mockCeremonies;
      try {
        const raw = localStorage.getItem(CEREMONIES_KEY);
        if (raw) storedCeremonies = JSON.parse(raw);
      } catch {
        storedCeremonies = mockCeremonies;
      }
      setCeremonies(storedCeremonies);
      
      // Auto-select first ceremony if available
      const savedCeremonyId = localStorage.getItem("selectedCeremonyId");
      if (savedCeremonyId && storedCeremonies.find((c) => c.id === savedCeremonyId)) {
        setSelectedCeremonyId(savedCeremonyId);
      } else if (storedCeremonies.length > 0) {
        setSelectedCeremonyId(storedCeremonies[0].id);
      }
  ...
  const createCeremony = async (name: string, date: string): Promise<Ceremony> => {
    ...
    setCeremonies((prev) => {
      const next = [...prev, newCeremony];
      localStorage.setItem(CEREMONIES_KEY, JSON.stringify(next));
      return next;
    });
  ...
  const updateCeremonyProgress = useCallback((step: keyof Ceremony["setupProgress"], completed: boolean) => {
    if (!selectedCeremony) return;

    setCeremonies((prev) => {
      const next = prev.map((c) =>
        c.id === selectedCeremony.id
          ? c.setupProgress[step] === completed
            ? c
            : {
                ...c,
                setupProgress: {
      ...
      if (next.every((c, index) => c === prev[index])) return prev;
      localStorage.setItem(CEREMONIES_KEY, JSON.stringify(next));
      return next;
    });
  }, [selectedCeremony]);
