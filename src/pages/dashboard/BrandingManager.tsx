...
import { saveBranding, getBranding, updateBranding } from "@/lib/brandingStore";
...
export default function BrandingManager() {
  ...
  // Royalty-free track URLs (CC0 / public domain samples)
  const LIBRARY_TRACKS: Record<string, string> = {
  ...
  // Hydrate from previously saved branding
  useEffect(() => {
    const b = getBranding();
    if (b.appliedAt) {
      if (b.logoUrl) setLogo({ name: b.logoName || "logo", size: "Saved", url: b.logoUrl });
      setPrimaryColor(b.primaryColor);
      setSecondaryColor(b.secondaryColor);
      setFontFamily(b.fontFamily);
      if (b.musicTrackUrl) setMusicTrack({ name: b.musicTrackName || "music", size: "Saved", url: b.musicTrackUrl });
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const onError = () => {
      setIsPlaying(false);
      toast({ title: "Couldn't play track", description: "The audio file failed to load.", variant: "destructive" });
    };
  ...
  // Sync src whenever the selected track changes
  useEffect(() => {
    const audio = audioRef.current;
    if (!audio) return;
    if (musicTrack?.url) {
      if (audio.src !== musicTrack.url) {
  ...
  const togglePlayback = async () => {
    const audio = audioRef.current;
    if (!audio || !musicTrack?.url) return;
    try {
      if (audio.paused) {
        await audio.play();
        setIsPlaying(true);
      ...
      setIsPlaying(false);
      toast({ title: "Playback blocked", description: "Click play again to start.", variant: "destructive" });
    }
  };
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!user) {
      setIsUploading(null);
      toast({ title: "Login required", description: "Please log in before uploading branding.", variant: "destructive" });
      return;
    }

    const ext = file.name.split(".").pop() || "png";
    const path = `${user.id}/branding/logo-${Date.now()}.${ext}`;
    const { error } = await supabase.storage.from("ceremony-videos").upload(path, file, { upsert: true, contentType: file.type });
    if (error) {
      setIsUploading(null);
    ...
    const { data } = supabase.storage.from("ceremony-videos").getPublicUrl(path);
    const uploadedLogo = { name: file.name, size: `${(file.size / 1024).toFixed(1)} KB`, url: data.publicUrl };
    setLogo(uploadedLogo);
    updateBranding({ logoUrl: uploadedLogo.url, logoName: uploadedLogo.name });
    setIsUploading(null);
    toast({ title: "Logo uploaded successfully!", description: "Your logo is saved and will not disappear when changing pages." });
  };

  const handleRemoveLogo = () => {
    setLogo(null);
    updateBranding({ logoUrl: null, logoName: null });
  ...
  const handleMusicUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!user) {
      setIsUploading(null);
      toast({ title: "Login required", description: "Please log in before uploading music.", variant: "destructive" });
      return;
    }

    const ext = file.name.split(".").pop() || "mp3";
    const path = `${user.id}/branding/music-${Date.now()}.${ext}`;
    const { error } = await supabase.storage.from("ceremony-videos").upload(path, file, { upsert: true, contentType: file.type });
    if (error) {
      setIsUploading(null);
    ...
    const { data } = supabase.storage.from("ceremony-videos").getPublicUrl(path);
    setMusicTrack({ name: file.name, size: `${(file.size / (1024 * 1024)).toFixed(2)} MB`, url: data.publicUrl });
    updateBranding({ musicTrackName: file.name, musicTrackUrl: data.publicUrl });
    setIsUploading(null);
    toast({
      title: "Music uploaded successfully!",
      description: "Your background music is saved and will not disappear when changing pages.",
    });
  };
  ...
  const handleRemoveMusic = () => {
    ...
    setSelectedLibraryTrack(null);
    setIsPlaying(false);
    updateBranding({ musicTrackName: null, musicTrackUrl: null });
  ...
  const handleSelectLibraryTrack = (track: string) => {
    ...
    setMusicTrack({
      name: `${track}.mp3`,
      size: "Royalty-Free",
      url: LIBRARY_TRACKS[track],
    });
    updateBranding({ musicTrackName: `${track}.mp3`, musicTrackUrl: LIBRARY_TRACKS[track] });
    toast({
  ...
  const handleSave = async () => {
    ...
    saveBranding({
      ...
      musicTrackUrl: musicTrack?.url ?? null,
    });
    updateCeremonyProgress("branding", true);
    setIsSaving(false);
    toast({
      title: "Branding applied to all clips!",
      description: "Open Clip Generation to see your logo, colors and music on every winner clip.",
    });
  };
      ...
      <div className="space-y-6">
        ...
        >
          ...
          <div className="flex gap-3">
            ...
            <Button variant="secondary" onClick={() => { updateCeremonyProgress("branding", true); navigate("/dashboard/livestream"); }}>
              Next: Livestream
