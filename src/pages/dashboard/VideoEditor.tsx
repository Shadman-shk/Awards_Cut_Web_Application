...
const MOCK_CLIPS: ClipRow[] = [
  { id: "mock-1", winner_name: "Sarah Johnson", category: "Best Innovation", duration_label: "0:32", status: "ready", recipient_email: "sarah@example.com", recipient_phone: "+15551112222", start_offset_seconds: 0, end_offset_seconds: 32, music_track: null, preview_url: null, source_video_url: null },
  { id: "mock-2", winner_name: "Michael Chen", category: "Leadership Award", duration_label: "0:28", status: "ready", recipient_email: "michael@example.com", recipient_phone: "+15552223333", start_offset_seconds: 0, end_offset_seconds: 28, music_track: null, preview_url: null, source_video_url: null },
  { id: "mock-3", winner_name: "Emily Davis", category: "Rising Star", duration_label: "0:30", status: "processing", recipient_email: "emily@example.com", recipient_phone: "+15553334444", start_offset_seconds: 0, end_offset_seconds: 30, music_track: null, preview_url: null, source_video_url: null },
];

const MUSIC_TRACKS = [
  { id: "none", name: "No music" },
  { id: "uplifting-corporate", name: "Uplifting Corporate" },
  { id: "cinematic-triumph", name: "Cinematic Triumph" },
  { id: "warm-acoustic", name: "Warm Acoustic" },
  { id: "elegant-piano", name: "Elegant Piano" },
...
export default function VideoEditor() {
  ...
  // Load clips: mock list when Mock Mode is on, otherwise from DB
  useEffect(() => {
    let cancelled = false;
    const load = async () => {
      ...
      if (isMock || !user) {
        if (!cancelled) {
          setClips(MOCK_CLIPS);
          setSelectedClipId((prev) => prev ?? MOCK_CLIPS[0]?.id ?? null);
      ...
      const { data, error } = await supabase
        ...
        .select("id, winner_name, category, duration_label, status, recipient_email, recipient_phone, start_offset_seconds, end_offset_seconds, music_track, preview_url, source_video_url")
        .eq("user_id", user.id)
        .order("created_at", { ascending: false })
        .limit(50);
      if (cancelled) return;
      if (error) {
        toast({ title: "Couldn't load clips", description: error.message });
        setClips(MOCK_CLIPS);
      } else {
        const rows = (data || []) as ClipRow[];
        setClips(rows.length > 0 ? rows : MOCK_CLIPS);
      }
      setLoadingClips(false);
    };
    void load();
  ...
  const selectedClip = useMemo<ClipRow>(
    () => clips.find((c) => c.id === selectedClipId) || clips[0] || MOCK_CLIPS[0],
    [clips, selectedClipId]
  );

  // Trim state derived from selected clip
  const trimStart = selectedClip?.start_offset_seconds ?? 0;
  const trimEnd = selectedClip?.end_offset_seconds ?? 30;
  const duration = Math.max(trimEnd, trimStart + 1, 30);
  const musicTrack = selectedClip?.music_track ?? "none";
  ...
  const handleSendToWinner = async () => {
    ...
    toast({
      title: "Video sent!",
      description: `Video has been sent to ${selectedClip.winner_name} via ${methods.join(" and ")}.`,
    });
  };
              ...
              <div className="aspect-video bg-charcoal relative">
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-lg font-bold text-foreground">{selectedClip.winner_name}</p>
          ...
          <div className="space-y-4">
              ...
              <div className="space-y-2">
                {loadingClips ? (
                  ...
                  <p className="text-sm text-muted-foreground text-center py-4">
                    No clips yet. Generate clips from the Livestream or load Demo data.
                  </p>
                ) : clips.map((clip) => (
                  <motion.div
                    key={clip.id}
                  ...
                  >
                    <p className="font-medium text-foreground text-sm">{clip.winner_name}</p>
                    <p className="text-xs text-muted-foreground">{clip.category}</p>
                    <div className="flex items-center justify-between mt-2">
                      <span className="text-xs text-muted-foreground">{clip.duration_label || "—"}</span>
            ...
            >
              <div className="flex items-center justify-between">
                ...
                {savingEdit && <Loader2 className="h-3.5 w-3.5 animate-spin text-muted-foreground" />}
              </div>

              {/* Trim */}
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label className="text-xs text-muted-foreground">Trim range</Label>
                ...
                <Slider
                  ...
                  onValueChange={(v) => {
                    ...
                    setClips((prev) =>
                      prev.map((c) => c.id === selectedClip.id
                        ? { ...c, start_offset_seconds: v[0], end_offset_seconds: v[1] }
                        : c)
                    );
                  }}
                  onValueCommit={async (v) => {
                    if (v.length !== 2 || !selectedClip || isMock || selectedClip.id.startsWith("mock-")) {
                      toast({ title: "Trim updated", description: `${formatTime(v[0])} – ${formatTime(v[1])} (preview only)` });
                      return;
                    }
                    setSavingEdit(true);
                    const { error } = await supabase
                      .from("generated_clips")
                      .update({ start_offset_seconds: v[0], end_offset_seconds: v[1] })
                      .eq("id", selectedClip.id);
                    setSavingEdit(false);
                    if (error) toast({ title: "Couldn't save trim", description: error.message });
                    else toast({ title: "Trim saved", description: `${formatTime(v[0])} – ${formatTime(v[1])}` });
                  }}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={async (val) => {
                    setClips((prev) =>
                      prev.map((c) => c.id === selectedClip.id ? { ...c, music_track: val === "none" ? null : val } : c)
                    );
                    if (isMock || !selectedClip || selectedClip.id.startsWith("mock-")) {
                      toast({ title: "Music updated", description: MUSIC_TRACKS.find(t => t.id === val)?.name });
                      return;
                    }
                    setSavingEdit(true);
                    const { error } = await supabase
                      .from("generated_clips")
                      .update({ music_track: val === "none" ? null : val })
                      .eq("id", selectedClip.id);
                    setSavingEdit(false);
                    if (error) toast({ title: "Couldn't save music", description: error.message });
                    else toast({ title: "Music saved", description: MUSIC_TRACKS.find(t => t.id === val)?.name });
                  }}
                >
              ...
              <div className="grid grid-cols-2 gap-2 pt-1">
                <Button
                  variant="secondary"
                  size="sm"
                  onClick={() => toast({ title: "Swap source clip", description: "Open the Awards Manager to re-link a different source recording." })}
                >
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    if (isMock || selectedClip.id.startsWith("mock-")) {
                      setClips((prev) => prev.filter((c) => c.id !== selectedClip.id));
                      setSelectedClipId(null);
                      toast({ title: "Clip removed", description: "Mock clip removed from preview." });
                      return;
                    }
                    setSavingEdit(true);
                    const { error } = await supabase
                    ...
                    setSavingEdit(false);
                    if (error) {
                      toast({ title: "Couldn't remove clip", description: error.message });
                    } else {
                      setClips((prev) => prev.filter((c) => c.id !== selectedClip.id));
                      setSelectedClipId(null);
      ...
      <Dialog open={isSendModalOpen} onOpenChange={setIsSendModalOpen}>
        <DialogContent className="max-w-md bg-card border-border">
          <DialogHeader>
            ...
            <DialogDescription>
              Send the video directly to {selectedClip.winner_name}.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6 pt-4">
            ...
            <div className="p-4 rounded-xl bg-muted/50 border border-border/30">
              <p className="font-medium text-foreground">{selectedClip.winner_name}</p>
              <p className="text-sm text-muted-foreground">{selectedClip.category}</p>
              <div className="flex items-center gap-4 mt-3 text-sm text-muted-foreground">
                <span className="flex items-center gap-1">
                  <Mail className="h-4 w-4" />
                  {selectedClip.recipient_email || "—"}
                </span>
              </div>
            </div>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">{selectedClip.recipient_email || "—"}</p>
                  </div>
                </motion.div>
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">{selectedClip.recipient_phone || "—"}</p>
                  </div>
                </motion.div>
              </div>
            </div>
