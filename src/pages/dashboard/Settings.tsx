...
export default function Settings() {
  const { user } = useAuth();
  const { isEnabled: is2FAEnabled, isLoading: mfaLoading, refresh: refreshMFA, unenroll: unenrollMFA } = useMFA();
        ...
        <DialogContent className="max-w-md bg-card border-border/50">
          ...
          {is2FAEnabled ? (
            <div className="space-y-4 py-4">
              ...
              <div className="flex gap-3 justify-end">
                <Button variant="secondary" onClick={() => setIs2FAModalOpen(false)}>Close</Button>
                <Button variant="destructive" onClick={handleDisable2FA}>Disable 2FA</Button>
              </div>
            </div>
          ) : (
            <TwoFactorSetup 
              onComplete={() => { setIs2FAModalOpen(false); refreshMFA(); }}
