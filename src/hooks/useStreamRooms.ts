...
export function useStreamRooms() {
  ...
  const createRoom = useCallback(async (name: string) => {
    ...
    try {
      const { data: streamData, error: fnErr } = await supabase.functions.invoke("livepeer-stream", {
        body: { action: "create" },
      });
  ...
  const deleteRoom = useCallback(async (room: StreamRoom) => {
    ...
    if (room.livepeer_stream_id) {
      try {
        await supabase.functions.invoke("livepeer-stream", {
  ...
  const updateRoomStatus = useCallback(async (roomId: string, status: string) => {
    // CRITICAL: Always re-stamp started_at on every transition into a monitoring/live state.
    // Reusing a stale started_at from a previous session causes Livepeer clip windows to
    // resolve to negative/wrong offsets and produces empty or unrelated footage.
    ...
    await supabase.from("stream_rooms").update(patch as any).eq("id", roomId);
    setRooms(prev => prev.map(r => r.id === roomId
      ? {
          ...r,
          status,
          ...(patch.started_at ? { started_at: patch.started_at as string } : {}),
          ...(status === "live" || status === "monitoring" ? { ended_at: null } : {}),
          ...(patch.ended_at && (status === "idle" || status === "gone") ? { ended_at: patch.ended_at as string } : {}),
        } as any
      : r
    ));
  }, []);
  ...
  const checkRoomHealth = useCallback(async (room: StreamRoom) => {
    ...
    try {
      const { data } = await supabase.functions.invoke("livepeer-stream", {
