...
import {
  ...
  Settings2, Clock, Timer, ArrowRight,
} from "lucide-react";
...
import type { StreamRoom } from "@/hooks/useStreamRooms";
import { useUserRole } from "@/hooks/useUserRole";
import { useClipPipeline, type DetectionFrame } from "@/hooks/useClipPipeline";
...
import { useLiveFaceMatch } from "@/hooks/useLiveFaceMatch";
import { Scan } from "lucide-react";

type ClipPipeline = ReturnType<typeof useClipPipeline>;

function captureVideoFrame(video: HTMLVideoElement | null, sourceKind: DetectionFrame["sourceKind"]): DetectionFrame | null {
  if (!video) {
    console.warn("[detection] captureVideoFrame: no video element attached yet");
    return null;
  }
  if (video.readyState < HTMLMediaElement.HAVE_CURRENT_DATA) {
    console.warn("[detection] captureVideoFrame: video not ready (readyState=" + video.readyState + ")");
    return null;
  }
  if (video.videoWidth === 0 || video.videoHeight === 0) {
    console.warn("[detection] captureVideoFrame: zero video dimensions");
    return null;
  }
  ...
  try {
    ...
    if (!ctx) {
      console.warn("[detection] captureVideoFrame: 2d context unavailable");
      return null;
    }
    ctx.drawImage(video, 0, 0, canvas.width, canvas.height);
    const dataUrl = canvas.toDataURL("image/jpeg", 0.82);
    return { dataUrl, sourceKind };
  } catch (err) {
    console.error("[detection] captureVideoFrame failed (likely CORS-tainted canvas — make sure HLS video has crossOrigin='anonymous'):", err);
    return null;
  }
}

interface StreamRoomsGridProps {
  pipeline: ClipPipeline;
  rooms: StreamRoom[];
  loading: boolean;
  createRoom: (name: string) => Promise<StreamRoom | null>;
  deleteRoom: (room: StreamRoom) => Promise<void>;
  updateRoomName: (roomId: string, name: string) => Promise<void>;
  checkRoomHealth: (room: StreamRoom) => Promise<string | null>;
  updateRoomStatus: (roomId: string, status: string) => Promise<void>;
}

export function StreamRoomsGrid({ pipeline, rooms, loading, createRoom, deleteRoom, updateRoomName, checkRoomHealth, updateRoomStatus }: StreamRoomsGridProps) {
  ...
  // Poll only the active room and at a slower pace so preview playback stays smooth.
  useEffect(() => {
    const activeRoom = rooms.find(room => room.livepeer_stream_id && (room.status === "live" || room.status === "monitoring"));
    if (!activeRoom) return;
    const interval = setInterval(() => {
      void checkRoomHealth(activeRoom);
    }, 30000);
  ...
  const openRoomWithClips = () => {
    navigate("/dashboard/clip-generation?focus=latest");
  };
...
function StreamRoomCard({
  ...
  return (
    <div
      className={`rounded-2xl bg-card border overflow-hidden cursor-pointer transition-colors ${
...
function ExpandedRoomPanel({
  ...
  const handleLiveVideoElement = useCallback((el: HTMLVideoElement | null) => {
    liveVideoRef.current = el;
  }, []);

  // Face recognition: enrolled awardees + live matching while monitoring
  const { faces: enrolledFaces } = useAwardeeFaces();
  const activeVideoRef = inputMode === "upload" ? uploadVideoRef : liveVideoRef;
  const { currentMatch: liveFaceMatch, modelsReady: faceModelsReady } = useLiveFaceMatch(
    activeVideoRef,
    enrolledFaces,
    { enabled: isMonitoring && enrolledFaces.length > 0, intervalMs: 2000, threshold: 0.55 }
  );
  ...
  const handleStartMonitoring = async () => {
    const captureFrame = (): DetectionFrame | null => captureVideoFrame(
      inputMode === "upload" ? uploadVideoRef.current : liveVideoRef.current,
      inputMode === "upload" ? "upload" : "rtmp",
    );
    const startedAt = new Date().toISOString();
    setIsMonitoring(true);
    await onStatusChange("monitoring");
    const activeVideoUrl = inputMode === "upload" ? uploadedVideoUrl || roomVideoUrl : roomVideoUrl;
    pipeline.startMonitoring(`room-${room.id}-stream.mp4`, activeVideoUrl, {
      ...
      playbackId: inputMode === "rtmp" ? room.livepeer_playback_id || undefined : undefined,
  ...
  const openClipGeneration = () => navigate("/dashboard/clip-generation?focus=latest");

  const handleStopMonitoring = () => {
    setIsMonitoring(false);
    pipeline.stopMonitoring?.();
    onStatusChange("idle");
    toast({ title: "Monitoring Stopped", description: "AI detection turned off." });
  };
  ...
  return (
    <div className="rounded-2xl bg-card border border-primary/30 overflow-hidden">
      ...
      <div className="p-4 space-y-6">
        ...
        {/* Preview first, setup below so the stream never overlaps credentials */}
        <div className="grid grid-cols-1 gap-4">
          ...
          <div className="space-y-3">
            ...
            <div className="aspect-video rounded-xl bg-charcoal border border-border/50 relative overflow-hidden">
              ...
              {inputMode === "rtmp" && room.livepeer_playback_id ? (
                <HLSPlayer playbackId={room.livepeer_playback_id} muted className="w-full h-full object-contain" onVideoElement={handleLiveVideoElement} />
              ) : uploadedVideoUrl && inputMode === "upload" ? (
                <video ref={uploadVideoRef} src={uploadedVideoUrl} controls autoPlay playsInline muted className="absolute inset-0 w-full h-full object-contain" />
              ) : (
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    {inputMode === "rtmp" ? (
              ...
              {isMonitoring && (
                <div className="absolute bottom-2 left-2 flex flex-col gap-1 items-start">
                  <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-black/60 text-xs text-white">
                    <Brain className="h-3 w-3 text-primary animate-pulse" />
                    AI Active
                  </div>
                  {enrolledFaces.length > 0 && (
                    <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-black/60 text-[11px] text-white">
                      <Scan className={`h-3 w-3 ${faceModelsReady ? "text-emerald-400" : "text-muted-foreground"} ${faceModelsReady ? "animate-pulse" : ""}`} />
                      {faceModelsReady ? `Face match: ${enrolledFaces.length} enrolled` : "Loading face models…"}
                    </div>
                  )}
                  {liveFaceMatch && (
                    <motion.div
                      ...
                      className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-500/90 text-white text-xs font-semibold shadow-lg"
                    >
                      <Scan className="h-3 w-3" />
                      Recognized: {liveFaceMatch.awardeeName}
                    </motion.div>
                  )}
                </div>
              )}
            ...
            <div className="flex gap-2">
              {!isMonitoring ? (
                <Button variant="hero" size="sm" className="flex-1" onClick={handleStartMonitoring} disabled={(inputMode === "rtmp" && !room.livepeer_playback_id) || (inputMode === "upload" && !uploadedVideoUrl)}>
              ...
              <FaceEnrollmentPanel triggerLabel="Faces" />
            </div>
            ...
            {pipeline.isScanning && (
              <div className="p-3 rounded-xl bg-muted/50 border border-border/30">
                ...
                <div className="w-full h-1.5 rounded-full bg-muted overflow-hidden">
                  <div className="h-full rounded-full bg-primary transition-[width] duration-300" style={{ width: `${pipeline.scanProgress}%` }} />
                </div>
                <Button variant="secondary" size="sm" className="mt-3 w-full" onClick={openClipGeneration}>
                  <ArrowRight className="h-4 w-4" /> Open Clip Generation
                </Button>
              </div>
            )}
          </div>
              ...
              <UploadMode
                onVideoReady={(filename, videoUrl) => {
                  ...
                  pipeline.startPipeline(filename, videoUrl, {
                    ...
                    autoClipGeneration: settings.auto_clip_enabled,
                    confidenceThreshold: settings.confidence_threshold,
                    captureFrame: () => captureVideoFrame(uploadVideoRef.current, "upload"),
                  });
                }}
              />
            )}
        ...
        {/* Legacy AI Detection + Generated Clips panels removed.
            Use the AI Suggestions and Manual Trigger panels at the top of
            the Livestream Manager instead. */}
        {false && (
