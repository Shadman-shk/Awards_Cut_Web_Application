...
function captureVideoFrame(video: HTMLVideoElement | null) {
  ...
  try {
    ...
    return { dataUrl: canvas.toDataURL("image/jpeg", 0.82), sourceKind: "rtmp" as const };
  } catch {
    return null;
  }
}
  ...
  useEffect(() => {
    if (livepeer.livepeerStatus === "creating") store.setConnectionStatus("connecting");
      ...
      if (!pipeline.isScanning) {
        pipeline.startPipeline("rtmp-live-stream.mp4", store.streamData?.playbackUrl || undefined, {
          detectionInterval: store.detectionInterval,
          clipDuration: store.clipDuration,
          autoClipGeneration: store.autoClip,
          playbackId: store.streamData?.playbackId || undefined,
          captureFrame: () => captureVideoFrame(liveVideoRef.current),
        });
      }
          ...
          <div className="space-y-4">
            {playbackId && (
              <div className="sr-only" aria-hidden="true">
                <HLSPlayer playbackId={playbackId} muted onVideoElement={(el) => { liveVideoRef.current = el; }} />
              </div>
            )}
